...
def main(**opt) -> Tuple[str, tfs.TfsDataFrame]:
    ...
    Keyword Args:
        ...
        output (str/Path): Path to write command and tfs_df file.
                           Extension is ignored and replaced with '.tfs' and '.madx'
                           for the Dataframe and the command file respectively.
        rdts (list[str]): RDTs to correct.
                          Format: 'Fjklm', or starting with 'J' (skew) or 'K' (normal)
                          to force corrector type.
        rdts2 (list[str]): RDTs to correct for second beam/file, if different from first.
                           Same format rules as for `rdts`.
        accel (str): Which accelerator we have. (lhc, hllhc)
