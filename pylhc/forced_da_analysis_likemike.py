...
norm_emittance = 3.75 * 1e-6  # Design LHC

gamma = energy/proton_mass  # E = gamma * m0 * c^2
beta = np.sqrt(1 - (1 / gamma**2))
emittance = norm_emittance/(beta * gamma)

print(norm_emittance)
print(emittance)
...
filldata = db.getLHCFillData( FILL_NUMBER )
t1 = filldata['startTime']
t2 = filldata['endTime']


database = db
# database = pagestore.PageStore('./~temp/MD3603DataBase.db', './~temp/DataBase/')
# database.store( db.get( 'LHC.BSRT.5R4.B1:FIT_SIGMA_H' , t1, t2 ) )
# database.store( db.get( 'LHC.BSRT.5R4.B1:FIT_SIGMA_V' , t1, t2 ) )
x , y = database.get( 'LHC.BCTFR.A6R4.B1:BEAM_INTENSITY', t1, t2 )[ 'LHC.BCTFR.A6R4.B1:BEAM_INTENSITY' ]
fbct_df = pd.DataFrame( data=y, index=x, columns = ['INTENSITY'] )

x , y = database.get( 'LHC.BSRT.5R4.B1:BUNCH_EMITTANCE_H', t1, t2 )[ 'LHC.BSRT.5R4.B1:BUNCH_EMITTANCE_H' ]

if BUNCH_ID is None:
    pass  # find first non-zero column in y

bsrt_h_df = pd.DataFrame( data=y[:,BUNCH_ID], index=x, columns = ['EMITTANCE_H'] )

# remove entries with zero emittance as unphysical
bsrt_h_df['EMITTANCE_H_CLEAN'] = np.where( bsrt_h_df['EMITTANCE_H'] < 1., bsrt_h_df['EMITTANCE_H'].shift(1), bsrt_h_df['EMITTANCE_H'] )
