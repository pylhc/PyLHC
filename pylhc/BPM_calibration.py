...
def _get_beta_fit(positions, beta_values, beta_err):
    ...
    fit, fit_cov = curve_fit(beta_function,
                             positions,
                             beta_values,
                             p0=initial_values,
                             sigma=beta_err,
    ...
    beta_max_fit = beta_function(positions,
                                 fit[0] + fit_err[0],
                                 fit[1] + fit_err[1])
    beta_min_fit = beta_function(positions,
                                 fit[0] - fit_err[0],
                                 fit[1] - fit_err[1])
...
def _get_dispersion_from_phase(normalised_dispersion, beta):
    ...
    d_phase_err += (1/2 * normalised_dispersion['amp'] / np.sqrt(beta['phase']) * beta['phase_err']) ** 2
...
def _get_dispersion_fit(positions, dispersion_values, dispersion_err):
    ...
    fit, fit_cov = curve_fit(dispersion_function,
                             positions,
                             values,
                             sigma=err)
    # FIXME add sigma=err
    ...
    dispersion_max_fit = dispersion_function(positions,
                                             fit[0] + fit_err[0],
                                             fit[1] + fit_err[1])
    dispersion_min_fit = dispersion_function(positions,
                                       fit[0] - fit_err[0],
                                       fit[1] - fit_err[1])
...
def _get_calibration_factors_dispersion(ip, plane, beta_phase_tfs, dispersion_tfs, model_tfs):
    beam = _get_beam_from_model(model_tfs)
    ...
    normalised_dispersion['amp_err'] = dispersion_tfs.loc[bpms, f'{ERR}{ND}{plane}'].dropna()

    # Get the dispersion from phase
    dispersion['phase'], dispersion['phase_err'] = _get_dispersion_from_phase(normalised_dispersion, beta)
    ...
    # Fit the dispersion from amplitude and the dispersion from phase
    dispersion['amp_fit'], dispersion['amp_fit_err'] = _get_dispersion_fit(positions_fit, dispersion['amp'], dispersion['amp_err'])
    dispersion['phase_fit'], dispersion['phase_fit_err'] = _get_dispersion_fit(positions_fit, dispersion['phase'], dispersion['phase_err'])
