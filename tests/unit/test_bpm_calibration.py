...
def test_bad_args():
    with pytest.raises(ArgumentError) as e:
        calibration.main(inputdir="wat", outputdir="", ips=[1, 5])
...
def test_no_beta_tfs(tmp_path):
    with pytest.raises(FileNotFoundError) as e:
        calibration.main(inputdir=pathlib.Path("wat"), outputdir=tmp_path, ips=[1, 5])
...
def test_wrong_ip(tmp_path):
    with pytest.raises(ArgumentError) as e:
        calibration.main(inputdir=MEASUREMENTS / "for_beta", outputdir=tmp_path, ips=[15, 22])
...
def test_beta_equal(tmp_path):
    factors = calibration.main(
        inputdir=MEASUREMENTS / "same_beta", outputdir=tmp_path, method="beta"
    )
...
def test_missing_bpms(tmp_path):
    calibration.main(
        inputdir=MEASUREMENTS / "missing_bpms", outputdir=tmp_path, method="beta", ips=[1, 5]
    )
...
def test_number_in_out(tmp_path):
    ...
    factors = calibration.main(
        inputdir=MEASUREMENTS / "for_beta", outputdir=tmp_path, method="beta"
    )
...
def test_no_error_tracking(tmp_path):
    ...
    factors = calibration.main(inputdir=MEASUREMENTS / "tracking", outputdir=tmp_path, ips=[4])
    ...
    assert_series_equal(
        ir4_x_df["ERROR_CALIBRATION_FIT"], expected, atol=precision, check_names=False
    )
    assert_series_equal(
        ir4_y_df["ERROR_CALIBRATION_FIT"], expected, atol=precision, check_names=False
    )
